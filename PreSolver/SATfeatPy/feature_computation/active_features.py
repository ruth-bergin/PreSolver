...
from SATfeatPy.feature_computation.enums import VarState, ClauseState
        ...
        if num_literals == 2:
            ...
            for i in range(num_literals):
                try:
                    ...
                    raise ValueError(f"Outer list size: {len(num_bin_clauses_with_var)} "
                                     f"Outer list index: {abs(clause[i])}\nInner list size: {len(clause)} Inner list index: {i}")
