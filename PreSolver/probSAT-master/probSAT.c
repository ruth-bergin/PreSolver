...
char *atom;
/** The inputted initial value of the variables. Possible values are 0 or 1.*/
char *initAtom;
...
static inline void allocateMemory() {
	...
	// Added in for pre-solver info
	initAtom = (char*) malloc(sizeof(char) * (numVars + 1));
...
static inline int randomBinaryWithProbability(int binaryValue) {
    flipInput = (double)rand() / RAND_MAX;
    if (flipInput <= initProb) {
        return binaryValue;
    } else {
        return !binaryValue;
    }
...
static inline void parseInit(){
    if (!customInit) {
    return;}
	register int i;
	char c;
    ifp = NULL;
    ifp = fopen(initFileName, "r");
    if (ifp == NULL) {
        fprintf(stderr, "c Error: Not able to open the file: %s\n", initFileName);
        exit(-1);
    }
...
static inline void init() {
	...
	for (i = 1; i <= numVars; i++) {
	    if (customInit){atom[i] = randomBinaryWithProbability(initAtom[i]);}
	    else {atom[i] = rand() % 2;}
		breaks[i] = 0;
	}
	//pass through all clauses and apply the assignment previously generated
	for (i = 1; i <= numClauses; i++) {
		j = 0;
		while ((lit = clause[i][j])) {
